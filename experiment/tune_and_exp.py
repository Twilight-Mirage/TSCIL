...
import time
import os
from experiment.tune_config import (
    config_generic,
    config_model,
    config_cl,
    set_dropout,
    modify_config_accordingly,
)
...
def tune_cl_agent_on_val_tasks(config, args, cls_order):
    ...
    for run in range(args.runs_val):
        ...
        run_args = SimpleNamespace(
            **config["generic"], **config["model"], **config["agent"], **vars(args)
        )
        ...
        task_stream = IncrementalTaskStream(
            data=args.data, scenario=args.scenario, cls_order=cls_order, split="val"
        )
...
def tune_offline_on_val_tasks(config, args, cls_order):
    ...
    for run in range(args.runs_val):
        ...
        print(
            "-- Run_val {} | {} sec -- Test Acc: {} -- ".format(
                run, end - start, np.around(test_acc, decimals=2)
            )
        )
...
def tune_hyperparams_on_val_tasks(
    args, cls_order, config_generic={}, config_model={}, config_cl={}
):
    # Modify the config according to dataset and agent
    args, config_generic, config_model, config_cl = modify_config_accordingly(
        args, config_generic, config_model, config_cl
    )
    ...
    if args.agent == "Offline":
        ...
        trainable_with_resources = tune.with_resources(
            partial(tune_offline_on_val_tasks, args=args, cls_order=cls_order),
            resources=resources,
        )
    else:
        reporter = tune.CLIReporter(metric_columns=["end_avg_acc", "end_avg_fgt"])
        trainable_with_resources = tune.with_resources(
            partial(tune_cl_agent_on_val_tasks, args=args, cls_order=cls_order),
            resources=resources,
        )
    ...
    if args.agent == "Offline":
        best_trial = results.get_best_result(
            metric="offline_acc", mode="max", scope="last"
        )
    else:
        best_trial = results.get_best_result(
            metric="end_avg_acc", mode="max", scope="last"
        )
    ...
    best_params = dict(
        best_params["model"], **best_params["agent"], **best_params["generic"]
    )
    return best_params
...
def tune_and_experiment_multiple_runs(args):
    ...
    for run in range(args.runs):
        ...
        if (
            args.fix_order or args.reuse_best
        ) and run > 0:  # if class order is fixed, only tune params in the first run
            pass
        else:
            tune_args = args
            tune_args.tune = True
            tune_args.stream_split = "val"
            best_params = tune_hyperparams_on_val_tasks(
                tune_args, cls_order, config_generic, config_model, config_cl
            )
        ...
        task_stream = IncrementalTaskStream(
            data=args.data, scenario=args.scenario, cls_order=cls_order, split="exp"
        )
        ...
        if args.agent == "Offline":
            ...
            for i in range(n_tasks_exp):
                ...
                if i + 1 == n_tasks_exp and args.cf_matrix:
                    ...
                    agent.plot_cf_matrix(
                        path=cf_matrix_path, classes=np.arange(task_stream.n_classes)
                    )
    ...
    if args.agent == "Offline":
        ...
        print(
            "---- Offline Accuracy with 95% CI is {} ----".format(
                np.around(acc, decimals=2)
            )
        )
    else:
        avg_end_acc, avg_end_fgt, avg_cur_acc, avg_acc, avg_bwtp = compute_performance(
            Acc_multiple_run_valid
        )
        print(
            " Avg_End_Acc {} Avg_End_Fgt {} Avg_Cur_Acc {} Avg_Acc {} Avg_Bwtp {} \n".format(
                np.around(avg_end_acc, decimals=2),
                np.around(avg_end_fgt, decimals=2),
                np.around(avg_cur_acc, decimals=2),
                np.around(avg_acc, decimals=2),
    ...
    if args.agent == "Offline":
        ...
        print(
            "Avg_End_Acc {} Avg_End_Fgt {} Avg_Cur_Acc {} Avg_Acc {} Avg_Bwtp {}".format(
                np.around(avg_end_acc, decimals=2),
                np.around(avg_end_fgt, decimals=2),
                np.around(avg_cur_acc, decimals=2),
                np.around(avg_acc, decimals=2),
