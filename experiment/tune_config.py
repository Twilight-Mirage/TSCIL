...
config_default = {
    "scenario": "class",
    "runs": 5,
    "runs_val": 3,  # To increase the confidence of best_config
    "seed": 1234,
    "verbose": False,
    ...
    "ncm_classifier": True,  # Default: False
...
config_generic = {
    "lr": 1e-3,  # Tried with 1e-2 & 1e-4, never be the best. So we focus on lradj.
    "lradj": tune.grid_search(
        ["TST", "step10", "step15"]
    ),  # 'step25' seldom be the best, removed.
...
config_cl = {
    "LwF": {
        "lambda_kd_lwf": tune.grid_search([1, 1e1, 1e2, 1e3, 1e4]),
        "adaptive_weight": True,
    },
    "EWC": {"lambda_impt": tune.grid_search([1e2, 1e3, 1e4]), "ewc_mode": "separate"},
...
def modify_config_accordingly(args, config_generic, config_model, config_cl):
    ...
    if (
        args.agent in agents_replay or args.agent == "Offline"
    ):  # Set a larger patience for methods using replay
        args.patience = 20
    ...
    if args.data == "har":
        if args.agent == "DT2W":
            config_cl[args.agent]["lambda_kd_fmap"] = tune.grid_search(
                [1e-1, 1e-2]
            )  # 1e-3 never be the best
    ...
    if "grabmyo" in args.data:
        if args.agent == "ASER":
            config_cl[args.agent]["aser_n_smp_cls"] = tune.grid_search(
                [4, 8, 12]
            )  # Tried with 2, never be the best

        if args.agent == "MAS":
            config_cl[args.agent]["lambda_impt"] = tune.grid_search(
                [1, 10, 1e2, 1e3]
            )  # 1e4 never be the best

    if args.agent in ["Inversion", "Mnemonics"]:
        config_generic["lradj"] = (
            "TST"  # experiments showed 'TST' is always the best. Set it directly to reduce grid.
        )
    ...
    if args.agent == "GR":
        args.runs_val = (
            1  # Hyper params is trivial for GR's performance. Set to 1 to speed up.
        )
